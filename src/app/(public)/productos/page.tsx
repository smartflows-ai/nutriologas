...
export default async function ProductosPage({ searchParams }: { searchParams: { category?: string } }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold text-gray-900 mb-2">Productos y servicios</h1>
      <p className="text-gray-500 mb-8">Encuentra el plan o producto ideal para tu salud</p>

      {/* Filtros de categoría */}
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-8">
          <a href="/productos" className={`badge px-4 py-2 rounded-full border text-sm font-medium transition-colors ${!searchParams.category ? "bg-primary text-white border-primary" : "border-gray-300 text-gray-600 hover:border-primary hover:text-primary"}`}>
            Todos
          </a>
          {categories.map((cat) => (
            <a key={cat} href={`/productos?category=${cat}`} className={`badge px-4 py-2 rounded-full border text-sm font-medium transition-colors ${searchParams.category === cat ? "bg-primary text-white border-primary" : "border-gray-300 text-gray-600 hover:border-primary hover:text-primary"}`}>
              {cat}
            </a>
          ))}
        </div>
