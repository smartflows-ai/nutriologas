...
export default function SalesChart({ data }: Props) {
  ...
  const chartData = Object.entries(grouped).map(([date, total]) => ({
    date: new Date(date + "T12:00:00").toLocaleDateString(lang === "es" ? "es-MX" : "en-US", { month: "short", day: "numeric" }),
    total: Math.round(total),
  }));

  if (chartData.length === 0) return <p className="text-sm text-gray-400 py-8 text-center">{t.crm.charts.noSales}</p>;
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <AreaChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
        ...
        <Tooltip formatter={(v: number) => [`$${v.toLocaleString(lang === "es" ? "es-MX" : "en-US")}`, t.crm.charts.sales]} />
