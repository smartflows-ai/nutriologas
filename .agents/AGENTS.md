...
**Tech Stack:**
Next.js 14 · TypeScript · Tailwind CSS · Prisma ORM · PostgreSQL (Supabase) · NextAuth v4 · Anthropic Claude AI · Cloudinary · Conekta · PayPal · Stripe · n8n
...
- n8n generates AI content and publishes to Facebook/Instagram
...
## 7. Native AI Business Copilot (Claude / OpenRouter)
...
2. **Interactive 1-Click Prompt Triggers**: Hovering any KPI metric exposes a "Preguntar ↗" button that auto-submits a targeted analytical prompt to the copilot.
...
4. **Tool Execution Flow**:
   ...
   - API calls Claude/OpenRouter with tools in `src/lib/ai/tools.ts`
   - `execute-tool.ts` runs Prisma queries — **always strictly filtered by `tenantId`**
   - Claude interprets data and streams response back
...
## 8. n8n Automation (Social Campaigns)

1. n8n polls `GET /api/internal/campaigns/due` (campaigns with expired `nextPostAt`)
2. Generates images + text with AI
3. Publishes to Facebook & Instagram via Meta Graph API
4. Calls `PATCH /api/internal/campaigns/[id]/posted` to update `lastPostedAt` and recalculate `nextPostAt`
5. Saves post history in `SocialPost`

> `/api/internal/*` routes are **excluded from the middleware matcher** — they use their own API key auth, not NextAuth.
